# ============================================================================
# PRODUCTION SIMULATION - OLS-SCAD FINAL VERSION
...
# MATCHED TO WEE-SCAD FRAMEWORK FOR DIRECT COMPARISON
# ============================================================================
...
# ============================================================================
cat(separator(), "\n")
cat("PRODUCTION MODE: OLS-SCAD FINAL RESULTS\n")
cat("Configuration: 1000-500 with 96 cores\n")
cat(separator(), "\n\n")

N_ITERATIONS <- 1000       # MATCHED to WEE-SCAD
BOOT_ITERATIONS <- 500     # MATCHED to WEE-SCAD
N_LAMBDA <- 15             # MATCHED to WEE-SCAD
C_GRID_SIZE <- 5           # MATCHED to WEE-SCAD
N_CORES <- 96              # MATCHED to WEE-SCAD
MASTER_SEED <- 1762        # MATCHED to WEE-SCAD
...
# ============================================================================
# BOOTSTRAP CI FUNCTIONS - ALL 5 METHODS
# ============================================================================
...
# ============================================================================
# MAIN SIMULATION FUNCTION - OLS-SCAD
# ============================================================================
...
simulate_one_iteration <- function(g) {
  ...
  # =========================================================================
  # OLS-SCAD MAIN ESTIMATION
  # =========================================================================
  OLS_SCAD_SIC_values <- numeric(length(SCAD_grid))
  for (j in 1:length(SCAD_grid)) {
    gamma <- coordinate_descent_wls(x, y, initial_betas = NULL, 
                                    lambda = SCAD_grid[j], 
                                    mth = 2, 
    ...
    k <- sum(gamma$beta != 0)
    OLS_SCAD_SIC_values[j] <- SCAD_RSS + c_n * k
  }
  
  OLS_SCAD_optimal_lambda <- SCAD_grid[which.min(OLS_SCAD_SIC_values)]
  OLS_SCAD_gamma <- coordinate_descent_wls(x, y, initial_betas = NULL, 
                                           lambda = OLS_SCAD_optimal_lambda, 
                                           mth = 2, 
                                           indices = non_I_set_indices)
  ...
  OLS_SCAD_alpha <- OLS_SCAD_gamma$coefficients[1:(r+1)]
  OLS_SCAD_eta <- OLS_SCAD_gamma$coefficients[(r+2):(2*r+1)]
  
  OLS_SCAD_rho <- c(0, OLS_rho)
  OLS_SCAD_eta <- c(0, as.array(OLS_SCAD_eta))
  OLS_SCAD_alpha <- as.array(OLS_SCAD_alpha)
  
  OLS_SCAD_beta <- OLS_SCAD_alpha + OLS_SCAD_eta * (OLS_SCAD_rho - 1)
  
  # =========================================================================
  # BOOTSTRAP SECTION
  # =========================================================================
  bootstrap_OLS_SCAD_beta <- matrix(NA, nrow = BOOT_ITERATIONS, ncol = 5)
  bootstrap_OLS_SCAD_eta <- matrix(NA, nrow = BOOT_ITERATIONS, ncol = 4)
  ...
  for(h in 1:BOOT_ITERATIONS) {
    ...
    # OLS-SCAD with bootstrap weights
    OLS_SCAD_SIC_values <- numeric(length(boot_SCAD_grid))
    ...
    for (j in 1:length(boot_SCAD_grid)) {
      ...
      c_n <- max(log(n), log(log(n)) * log(p))
      k <- sum(gamma$beta != 0)
      OLS_SCAD_SIC_values[j] <- n * log(SCAD_RSS/n) + c_n * k
    }
    
    OLS_SCAD_optimal_lambda <- boot_SCAD_grid[which.min(OLS_SCAD_SIC_values)]
    
    boot_OLS_SCAD_gamma <- coordinate_descent_wls(x, y, 
                                                  lambda = OLS_SCAD_optimal_lambda, 
                                                  initial_betas = boot_OLS_gamma[-1], 
                                                  boot_weight = boot_weight, 
                                                  mth = 2, 
    ...
    boot_OLS_SCAD_alpha <- boot_OLS_SCAD_gamma$coefficients[1:(r+1)]
    boot_OLS_SCAD_eta <- boot_OLS_SCAD_gamma$coefficients[(r+2):(2*r+1)]
    boot_OLS_SCAD_eta <- c(0, as.array(boot_OLS_SCAD_eta))
    boot_OLS_SCAD_alpha <- as.array(boot_OLS_SCAD_alpha)
    
    boot_OLS_SCAD_beta <- boot_OLS_SCAD_alpha + boot_OLS_SCAD_eta * (boot_OLS_rho - 1)
    
    bootstrap_OLS_SCAD_beta[h,] <- boot_OLS_SCAD_beta[1:5]
    bootstrap_OLS_SCAD_eta[h,] <- boot_OLS_SCAD_eta[2:5]
  }
  ...
  # 1. Percentile Method
  OLS_SCAD_beta_CI_percentile <- matrix(NA, nrow = 2, ncol = 5)
  for (idx in 1:5) {
    ci <- percentile_ci(bootstrap_OLS_SCAD_beta[, idx], OLS_SCAD_beta[idx])
    OLS_SCAD_beta_CI_percentile[1, idx] <- ci$lower
    OLS_SCAD_beta_CI_percentile[2, idx] <- ci$upper
  }
  
  # 2. Universal Method
  OLS_SCAD_beta_CI_universal <- matrix(NA, nrow = 2, ncol = 5)
  for (idx in 1:5) {
    ci <- universal_ci(bootstrap_OLS_SCAD_beta[, idx], OLS_SCAD_beta[idx])
    OLS_SCAD_beta_CI_universal[1, idx] <- ci$lower
    OLS_SCAD_beta_CI_universal[2, idx] <- ci$upper
  }
  
  # 3. BCa Method
  OLS_SCAD_beta_CI_bca <- matrix(NA, nrow = 2, ncol = 5)
  for (idx in 1:5) {
    ci <- bca_ci(bootstrap_OLS_SCAD_beta[, idx], OLS_SCAD_beta[idx])
    OLS_SCAD_beta_CI_bca[1, idx] <- ci$lower
    OLS_SCAD_beta_CI_bca[2, idx] <- ci$upper
  }
  
  # 4. Efron Standard Method
  OLS_SCAD_beta_CI_efron_standard <- matrix(NA, nrow = 2, ncol = 5)
  for (idx in 1:5) {
    ci <- efron_standard_ci(bootstrap_OLS_SCAD_beta[, idx], OLS_SCAD_beta[idx])
    OLS_SCAD_beta_CI_efron_standard[1, idx] <- ci$lower
    OLS_SCAD_beta_CI_efron_standard[2, idx] <- ci$upper
  }
  
  # 5. Efron Reflection Method
  OLS_SCAD_beta_CI_efron_reflection <- matrix(NA, nrow = 2, ncol = 5)
  for (idx in 1:5) {
    ci <- efron_reflection_ci(bootstrap_OLS_SCAD_beta[, idx], OLS_SCAD_beta[idx])
    OLS_SCAD_beta_CI_efron_reflection[1, idx] <- ci$lower
    OLS_SCAD_beta_CI_efron_reflection[2, idx] <- ci$upper
  }
  
  # For ETA: use percentile method
  OLS_SCAD_eta_CI_percentile <- matrix(NA, nrow = 2, ncol = 4)
  for (idx in 1:4) {
    ci <- percentile_ci(bootstrap_OLS_SCAD_eta[, idx], OLS_SCAD_eta[idx+1])
    OLS_SCAD_eta_CI_percentile[1, idx] <- ci$lower
    OLS_SCAD_eta_CI_percentile[2, idx] <- ci$upper
  }
  ...
  OLS_SCAD_beta_coverage_percentile <- (OLS_SCAD_beta_CI_percentile[1,] <= true_beta_vec) & 
    (OLS_SCAD_beta_CI_percentile[2,] >= true_beta_vec)
  
  OLS_SCAD_beta_coverage_universal <- (OLS_SCAD_beta_CI_universal[1,] <= true_beta_vec) & 
    (OLS_SCAD_beta_CI_universal[2,] >= true_beta_vec)
  
  OLS_SCAD_beta_coverage_bca <- (OLS_SCAD_beta_CI_bca[1,] <= true_beta_vec) & 
    (OLS_SCAD_beta_CI_bca[2,] >= true_beta_vec)
  
  OLS_SCAD_beta_coverage_efron_standard <- (OLS_SCAD_beta_CI_efron_standard[1,] <= true_beta_vec) & 
    (OLS_SCAD_beta_CI_efron_standard[2,] >= true_beta_vec)
  
  OLS_SCAD_beta_coverage_efron_reflection <- (OLS_SCAD_beta_CI_efron_reflection[1,] <= true_beta_vec) & 
    (OLS_SCAD_beta_CI_efron_reflection[2,] >= true_beta_vec)
  
  OLS_SCAD_eta_coverage <- (OLS_SCAD_eta_CI_percentile[1,] <= true_eta_vec) & 
    (OLS_SCAD_eta_CI_percentile[2,] >= true_eta_vec)
  ...
  OLS_SCAD_beta_fn <- sum(true_beta_nonzero & (abs(OLS_SCAD_beta[1:5]) < 1e-6))
  OLS_SCAD_beta_fp <- sum(!true_beta_nonzero & (abs(OLS_SCAD_beta[1:5]) >= 1e-6))
  OLS_SCAD_eta_fn <- sum(true_eta_nonzero & (abs(OLS_SCAD_eta[2:5]) < 1e-6))
  OLS_SCAD_eta_fp <- sum(!true_eta_nonzero & (abs(OLS_SCAD_eta[2:5]) >= 1e-6))
  
  # =========================================================================
  # BOOTSTRAP STATISTICS
  # =========================================================================
  
  OLS_SCAD_beta_boot_mean <- colMeans(bootstrap_OLS_SCAD_beta, na.rm = TRUE)
  OLS_SCAD_beta_boot_sd <- apply(bootstrap_OLS_SCAD_beta, 2, sd, na.rm = TRUE)
  OLS_SCAD_eta_boot_mean <- colMeans(bootstrap_OLS_SCAD_eta, na.rm = TRUE)
  OLS_SCAD_eta_boot_sd <- apply(bootstrap_OLS_SCAD_eta, 2, sd, na.rm = TRUE)
  
  return(list(
    OLS_SCAD_beta = OLS_SCAD_beta[1:5],
    OLS_SCAD_eta = OLS_SCAD_eta[2:5],
    
    # Coverage for all five methods
    OLS_SCAD_beta_coverage_percentile = OLS_SCAD_beta_coverage_percentile,
    OLS_SCAD_beta_coverage_universal = OLS_SCAD_beta_coverage_universal,
    OLS_SCAD_beta_coverage_bca = OLS_SCAD_beta_coverage_bca,
    OLS_SCAD_beta_coverage_efron_standard = OLS_SCAD_beta_coverage_efron_standard,
    OLS_SCAD_beta_coverage_efron_reflection = OLS_SCAD_beta_coverage_efron_reflection,
    OLS_SCAD_eta_coverage = OLS_SCAD_eta_coverage,
    
    # CI bounds for all methods
    OLS_SCAD_beta_CI_percentile_lower = OLS_SCAD_beta_CI_percentile[1, ],
    OLS_SCAD_beta_CI_percentile_upper = OLS_SCAD_beta_CI_percentile[2, ],
    OLS_SCAD_beta_CI_universal_lower = OLS_SCAD_beta_CI_universal[1, ],
    OLS_SCAD_beta_CI_universal_upper = OLS_SCAD_beta_CI_universal[2, ],
    OLS_SCAD_beta_CI_bca_lower = OLS_SCAD_beta_CI_bca[1, ],
    OLS_SCAD_beta_CI_bca_upper = OLS_SCAD_beta_CI_bca[2, ],
    OLS_SCAD_beta_CI_efron_standard_lower = OLS_SCAD_beta_CI_efron_standard[1, ],
    OLS_SCAD_beta_CI_efron_standard_upper = OLS_SCAD_beta_CI_efron_standard[2, ],
    OLS_SCAD_beta_CI_efron_reflection_lower = OLS_SCAD_beta_CI_efron_reflection[1, ],
    OLS_SCAD_beta_CI_efron_reflection_upper = OLS_SCAD_beta_CI_efron_reflection[2, ],
    
    # Variable selection metrics
    OLS_SCAD_beta_fn = OLS_SCAD_beta_fn,
    OLS_SCAD_beta_fp = OLS_SCAD_beta_fp,
    OLS_SCAD_eta_fn = OLS_SCAD_eta_fn,
    OLS_SCAD_eta_fp = OLS_SCAD_eta_fp,
    
    # Bootstrap statistics
    OLS_SCAD_beta_boot_mean = OLS_SCAD_beta_boot_mean,
    OLS_SCAD_beta_boot_sd = OLS_SCAD_beta_boot_sd,
    OLS_SCAD_eta_boot_mean = OLS_SCAD_eta_boot_mean,
    OLS_SCAD_eta_boot_sd = OLS_SCAD_eta_boot_sd
  ))
...
# For BETA values
OLS_SCAD_beta_df <- do.call(rbind, lapply(results, function(x) x$OLS_SCAD_beta))
OLS_SCAD_beta_mean <- round(colMeans(OLS_SCAD_beta_df), 3)
OLS_SCAD_beta_sd <- round(apply(OLS_SCAD_beta_df, 2, sd), 3)

# For ETA values
OLS_SCAD_eta_df <- do.call(rbind, lapply(results, function(x) x$OLS_SCAD_eta))
OLS_SCAD_eta_mean <- round(colMeans(OLS_SCAD_eta_df), 3)
OLS_SCAD_eta_sd <- round(apply(OLS_SCAD_eta_df, 2, sd), 3)

cat("OLS_SCAD_beta_mean:", paste(OLS_SCAD_beta_mean, collapse = ", "), "\n")
cat("OLS_SCAD_beta_sd:", paste(OLS_SCAD_beta_sd, collapse = ", "), "\n\n")

cat("OLS_SCAD_eta_mean:", paste(OLS_SCAD_eta_mean, collapse = ", "), "\n")
cat("OLS_SCAD_eta_sd:", paste(OLS_SCAD_eta_sd, collapse = ", "), "\n\n")
...
cat("\n", separator(110), "\n")
cat("OLS-SCAD COVERAGE PROBABILITY ANALYSIS (ALL 5 METHODS)\n")
cat(separator(110), "\n\n")

OLS_SCAD_beta_coverage_percentile_prob <- colMeans(do.call(rbind, lapply(results, function(x) x$OLS_SCAD_beta_coverage_percentile)))
OLS_SCAD_beta_coverage_universal_prob <- colMeans(do.call(rbind, lapply(results, function(x) x$OLS_SCAD_beta_coverage_universal)))
OLS_SCAD_beta_coverage_bca_prob <- colMeans(do.call(rbind, lapply(results, function(x) x$OLS_SCAD_beta_coverage_bca)))
OLS_SCAD_beta_coverage_efron_standard_prob <- colMeans(do.call(rbind, lapply(results, function(x) x$OLS_SCAD_beta_coverage_efron_standard)))
OLS_SCAD_beta_coverage_efron_reflection_prob <- colMeans(do.call(rbind, lapply(results, function(x) x$OLS_SCAD_beta_coverage_efron_reflection)))
OLS_SCAD_eta_coverage_prob <- colMeans(do.call(rbind, lapply(results, function(x) x$OLS_SCAD_eta_coverage)))
...
for(i in 1:5) {
  cat(sprintf("Beta[%d]    | %11.3f | %11.3f | %11.3f | %13.3f | %13.3f", 
              i, 
              OLS_SCAD_beta_coverage_percentile_prob[i],
              OLS_SCAD_beta_coverage_universal_prob[i],
              OLS_SCAD_beta_coverage_bca_prob[i],
              OLS_SCAD_beta_coverage_efron_standard_prob[i],
              OLS_SCAD_beta_coverage_efron_reflection_prob[i]))
...
for(i in 1:4) {
  cat("Eta[", i, "]: ", sprintf("%.3f", OLS_SCAD_eta_coverage_prob[i]), "\n", sep="")
}
cat("\n")
...
percentile_pass <- sum(OLS_SCAD_beta_coverage_percentile_prob >= 0.935)
universal_pass <- sum(OLS_SCAD_beta_coverage_universal_prob >= 0.935)
bca_pass <- sum(OLS_SCAD_beta_coverage_bca_prob >= 0.935)
efron_std_pass <- sum(OLS_SCAD_beta_coverage_efron_standard_prob >= 0.935)
efron_refl_pass <- sum(OLS_SCAD_beta_coverage_efron_reflection_prob >= 0.935)
...
percentile_dev <- mean(abs(OLS_SCAD_beta_coverage_percentile_prob - 0.95))
universal_dev <- mean(abs(OLS_SCAD_beta_coverage_universal_prob - 0.95))
bca_dev <- mean(abs(OLS_SCAD_beta_coverage_bca_prob - 0.95))
efron_std_dev <- mean(abs(OLS_SCAD_beta_coverage_efron_standard_prob - 0.95))
efron_refl_dev <- mean(abs(OLS_SCAD_beta_coverage_efron_reflection_prob - 0.95))
...
OLS_SCAD_beta_boot_mean_avg <- rowMeans(do.call(cbind, lapply(results, function(x) x$OLS_SCAD_beta_boot_mean)))
OLS_SCAD_beta_boot_sd_avg <- rowMeans(do.call(cbind, lapply(results, function(x) x$OLS_SCAD_beta_boot_sd)))

# Calculate bias
beta_bias <- OLS_SCAD_beta_boot_mean_avg - true_beta

cat("=== BIAS ANALYSIS FOR BETA PARAMETERS ===\n\n")
cat(sprintf("%-10s | %-10s | %-10s | %-10s | %-10s\n", "Parameter", "True", "Boot Mean", "Bias", "Bias/SD"))
cat(paste(rep("-", 60), collapse=""), "\n")
for(i in 1:5) {
  bias_ratio <- beta_bias[i] / OLS_SCAD_beta_boot_sd_avg[i]
  cat(sprintf("Beta[%d]    | %10.3f | %10.3f | %10.4f | %10.4f\n", 
              i, true_beta[i], OLS_SCAD_beta_boot_mean_avg[i], 
...
for(i in 1:5) {
  ...
  if(any(coverages < 0.935)) {
    ...
    cat("     Boot mean:         ", sprintf("%.3f", OLS_SCAD_beta_boot_mean_avg[i]), "\n")
    cat("     Boot SD:           ", sprintf("%.3f", OLS_SCAD_beta_boot_sd_avg[i]), "\n")
    cat("     Bias:              ", sprintf("%.4f", beta_bias[i]), "\n")
    cat("     Bias/SD:           ", sprintf("%.4f", beta_bias[i]/OLS_SCAD_beta_boot_sd_avg[i]), "\n\n")
  }
}

# ETA parameters with poor coverage
poor_eta_indices <- which(OLS_SCAD_eta_coverage_prob < 0.935)
if(length(poor_eta_indices) > 0) {
  cat("ETA parameters with poor coverage:\n\n")
  OLS_SCAD_eta_boot_mean_avg <- rowMeans(do.call(cbind, lapply(results, function(x) x$OLS_SCAD_eta_boot_mean)))
  OLS_SCAD_eta_boot_sd_avg <- rowMeans(do.call(cbind, lapply(results, function(x) x$OLS_SCAD_eta_boot_sd)))
  
  for(idx in poor_eta_indices) {
    cat("   Eta[", idx, "]:\n", sep="")
    cat("     Coverage:    ", sprintf("%.3f", OLS_SCAD_eta_coverage_prob[idx]), "\n")
    cat("     True value:  ", sprintf("%.3f", true_eta[idx]), "\n")
    cat("     Boot mean:   ", sprintf("%.3f", OLS_SCAD_eta_boot_mean_avg[idx]), "\n")
    cat("     Boot SD:     ", sprintf("%.3f", OLS_SCAD_eta_boot_sd_avg[idx]), "\n\n")
  }
} else {
...
OLS_SCAD_beta_mse <- round(mapply(mse, as.data.frame(OLS_SCAD_beta_df), true_beta), 4)

cat("BETA MSE:\n")
for(i in 1:5) {
  cat("Beta[", i, "]: ", OLS_SCAD_beta_mse[i], "\n", sep="")
}
cat("\n")

# Variable selection
OLS_SCAD_beta_FN_total <- sum(sapply(results, function(x) x$OLS_SCAD_beta_fn))
OLS_SCAD_beta_FP_total <- sum(sapply(results, function(x) x$OLS_SCAD_beta_fp))
OLS_SCAD_eta_FN_total <- sum(sapply(results, function(x) x$OLS_SCAD_eta_fn))
OLS_SCAD_eta_FP_total <- sum(sapply(results, function(x) x$OLS_SCAD_eta_fp))

cat("VARIABLE SELECTION:\n")
cat("BETA - False Negatives:", OLS_SCAD_beta_FN_total, ", False Positives:", OLS_SCAD_beta_FP_total, "\n")
cat("ETA  - False Negatives:", OLS_SCAD_eta_FN_total, ", False Positives:", OLS_SCAD_eta_FP_total, "\n\n")
